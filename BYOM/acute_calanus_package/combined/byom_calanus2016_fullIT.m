%% BYOM, byom_calanus2016_fullIT.m GUTS
%
...
glo.sel  = 2; % select death mechanism: 1) SD 2) IT
...
par.keS    = [   0.26974  1       0.01         10 1]; % elimination rate constant for structure (d-1)
par.kuS    = [    1084.3  1       0.01      1e+06 1]; % uptake rate constant for structure (L/L/d)
par.keL    = [   0.69118  1       0.01         10 1]; % elimination rate constant for lipids (d-1)
par.kuL    = [   0.78627  1       0.01      1e+06 1]; % uptake rate constant for lipids (L/L/d)
par.mS     = [     13488  1          0      1e+06 1]; % median threshold for survival (umol/L)
par.hb     = [ 0.0043801  1          0      1e+06 1]; % background hazard rate (1/d)
par.bS     = [         1  1          0       1000 1]; % killing rate (L/umol/d)
par.Fs     = [    1.6213  1          1        100 1]; % fraction spread of NEC distribution (-) (IT and mixed)
...
Tend = [1:0.2:3 3.5 4 5 6]; % times at which to calculate LCx, relative to control
