...
Xout(:,3) = (Xout(:,1)+FLS*Xout(:,2))/(1+FLS); % make the third state the total concentration
...
% the 'events'. The events function is used here to catch the depuration
% phase. This is not such a great solution, but it works well enough.
% Better to break up the time vector and solve in two parts (this strategy
% is used in the TKTD packages for BYOM).
%
...
if c == 1.2
    value(1)   = t - 4; % catch depuration phase
end
